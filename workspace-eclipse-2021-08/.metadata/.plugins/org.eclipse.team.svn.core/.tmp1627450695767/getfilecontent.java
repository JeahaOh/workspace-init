...
public class LclChCaServiceImpl extends EgovAbstractServiceImpl implements LclChCaService {
	...
	public List<LclChCaVO> getCpmsapi030(String stcode)	throws Exception {
		...
		try {
			...
			String url = "http://api.childcare.go.kr/mediate/rest/cpmsapi030/cpmsapi030/request?key=0584c598c8b54ab09684bb05f42185e6&arcode=" + stcode;
	        ...
	        for(int i=0; i<itemList.getLength();i++){
	        	...
	        	String crstatusname = "";
	    		for(Node node = itemList.item(i).getFirstChild(); node!=null; node=node.getNextSibling()){ //첫번째 자식을 시작으로 마지막까지 다음 형제를 실행
            	...
            	if(!"휴지".equals(crstatusname) && !"폐지".equals(crstatusname)) insertLclChCa(lclChCa);
