...
//	}
	...
	public Map<String, List<MvoteRespond>> selectRspnsTotList(int mvoteNo) throws Exception {
		...
		for(int i = 0;i < pnlgrpSexRspnsList.size();i++){
			...
			if (Double.isNaN(totPt))pnlgrpSexRspnsList.get(i).setTotPt(0);
			else pnlgrpSexRspnsList.get(i).setTotPt(totPt);
		}
		...
		for(int i = 0;i < selectSexRspnsList.size();i++){
			double mtotPt = Math.round((double)selectSexRspnsList.get(i).getmCount()/(double)totCnt * 100.0);
			double ftotPt = Math.round((double)selectSexRspnsList.get(i).getfCount()/(double)totCnt * 100.0);
			if (Double.isNaN(mtotPt))selectSexRspnsList.get(i).setMpt(0);
			else selectSexRspnsList.get(i).setMpt(mtotPt);
			if (Double.isNaN(ftotPt))selectSexRspnsList.get(i).setFpt(0);
			else selectSexRspnsList.get(i).setFpt(ftotPt);
		}
		...
		for(int i = 0;i < cnt.length;i++){
			...
			ageRspns2.setCode(" ");
			if(i==0)ageRspns2.setTotName("10세미만");else if(i==1)ageRspns2.setTotName("10~19세");else if(i==2)ageRspns2.setTotName("20~29세");else if(i==3)ageRspns2.setTotName("30~39세");
			else if(i==4)ageRspns2.setTotName("40~49세");else if(i==5)ageRspns2.setTotName("50~59세");else if(i==6)ageRspns2.setTotName("60세이상");
			ageRspns2.setTotCnt(Integer.parseInt(cnt[i]));
			totPt = Math.round((double)ageRspns2.getTotCnt()/(double)totCnt * 100.0);
			if (Double.isNaN(totPt))ageRspns2.setTotPt(0);
			else ageRspns2.setTotPt(totPt);
		...
		for(int i = 0;i < areaRspnsList.size();i++){
			...
			if (Double.isNaN(totPt))areaRspnsList.get(i).setTotPt(0);
			else areaRspnsList.get(i).setTotPt(totPt);
		}
